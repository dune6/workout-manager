...
func (s *Server) Register(register IRegister) http.HandlerFunc {
...
func (s *Server) Login(login ILogin) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		findUser, err := login.Login(input.Username, input.Password)
		if err != nil {
			switch {
			case errors.Is(err, database.ErrorUserNotFound):
